...
FROM ARTICULOS
WHERE EXISTENCIAS IS NOT NULL AND EXISTENCIAS = (SELECT MIN(EXISTENCIAS) 
						 FROM ARTICULOS)
/
...
FROM ARTICULOS
WHERE PR_COST = (SELECT MIN(PRECIO)
		 FROM LINEAS)
/

SELECT ARTICULO, PROVEEDOR, PR_COST
FROM ARTICULOS
WHERE PR_COST <= ALL(SELECT MIN(PR_VENT - PR_COST) 
		     FROM ARTICULOS)
...
FROM ARTICULOS
WHERE PR_VENT - PR_COST = (SELECT MAX(PR_VENT - PR_COST)
			   FROM ARTICULOS)
/

SELECT ARTICULO, PROVEEDOR, PR_VENT - PR_COST "BENEFICIO MAXIMO"
FROM ARTICULOS
WHERE PR_VENT - PR_COST) >= ALL(SELECT PR_VENT - PR_COST 
				FROM ARTICULOS)
/
...
SELECT *
FROM ALBARANES
WHERE FECHA_ALBARAN = (SELECT MAX(FECHA_ALBARAN)
		       FROM ALBARANES)
/

SELECT *
FROM ALBARANES
WHERE FECHA_ALBARAN >= ALL(SELECT FECHA_ALBARAN 
			   FROM ALBARANES)
/
...
WHERE PROVINCIA IN(SELECT PROVINCIA 
		   FROM PROVINCIAS 
		   WHERE UPPER(DESCRIPCION) LIKE'%D')
     AND EXISTENCIAS - BAJO_MINIMO = (SELECT MAX(EXISTENCIAS - BAJO_MINIMO) 
				      FROM ARTICULOS)
...
AND PROVINCIA IN (SELECT PROVINCIA 
		  FROM PROVINCIAS 
		  WHERE UPPER(SUBSTR(DESCRIPCION, 1, 1)) != 'M')
/
/* 29. Mostrar los clientes que no hayan pagado al contado sus pedidos en el mes de marzo de 1988. */

SELECT DISTINCT CLIENTE
FROM ALBARANES
WHERE TO_CHAR(FECHA_PAGO, 'MM/YYYY') = '03/1988'
  AND FORMPAGO <> ALL(SELECT FORMPAGO
		      FROM FORMPAGOS
		      WHERE UPPER(DESCRIPCION) = 'CONTADO')
/
...
WHERE NOT EXISTS(SELECT *
		 FROM ALBARANES
	    	 WHERE CLIENTES.CLIENTE = ALBARANES.CLIENTE
	    	   AND TO_CHAR(FECHA_PAGO, 'MM/YYYY') = '03/1988'
	    	   AND FORMPAGO = 'C')
	    AND EXISTS(SELECT *
	    	       FROM ALBARANES
		       WHERE CLIENTES.CLIENTE = ALBARANES.CLIENTE
...
WHERE EXISTS(SELECT *
	     ...
	     WHERE ALBARANES.FORMPAGO = FORMPAGOS.FORMPAGO
	       ...
	       AND TO_CHAR(FECHA_ALBARAN, 'MM/YYYY') = '03/1988'
	       AND UPPER(DESCRIPCION) != 'CONTADO')
/

SELECT *
FROM CLIENTES, FORMPAGOS, ALBARANES 
WHERE CLIENTES.CLIENTE = ALBARANES.CLIENTE
  AND FORMPAGOS.FORMPAGO = ALBARANES.FORMPAGO
  AND UPPER(FORMPAGOS.DESCRIPCION) != 'CONTADO'
  AND TO_CHAR(FECHA_ALBARAN, 'MM/YYYY') ='03/1988'
/
...
WHERE EXISTS (SELECT * 
	      FROM ALBARANES 
	      WHERE CLIENTES.CLIENTE = ALBARANES.CLIENTE
		AND ALBARANES.FORMPAGO != CLIENTES.FORMPAGO)
/

SELECT DISTINCT CLIENTES.*
FROM CLIENTES, ALBARANES
WHERE CLIENTES.CLIENTE = ALBARANES.CLIENTE
  AND CLIENTES.FORMPAGO != ALBARANES.FORMPAGO
/

SELECT CLIENTES.CLIENTE, CLIENTES.EMPRESA
FROM CLIENTES
WHERE CLIENTES.CLIENTE = ANY(SELECT ALBARANES.CLIENTE
			     FROM ALBARANES
			     WHERE ALBARANES.FORMPAGO != CLIENTES.FORMPAGO
...
SELECT *
FROM ALBARANES
WHERE ALBARAN NOT IN(SELECT ALBARAN 
		     FROM LINEAS)
/
...
WHERE CLIENTE IN(SELECT CLIENTE 
		 FROM ALBARANES 
		 WHERE FECHA_ALBARAN BETWEEN TO_DATE('1/1/1988', 'DD/MM/YYYY') 
		   AND TO_DATE('31/3/1988', 'DD/MM/YYYY'))
/
