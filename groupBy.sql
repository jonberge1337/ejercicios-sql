...
SELECT LINEAS.ALBARAN, ALBARANES.CLIENTE, PROVINCIAS.DESCRIPCION, ALBARANES.FECHA_ENVIO,
(sum((CANTIDAD * PRECIO) - (CANTIDAD * PRECIO * DESCUENTO / 100)) * 1.16) TOT_FRA
FROM ALBARANES, LINEAS, CLIENTES.PROVINCIA
WHERE ALBARANES.ALBARAN = LINEAS.ALBARAN
  AND ALBARANES.CLIENTE = CLIENTES.CLIENTE
  AND CLIENTES.PROVINCIA = PROVINCIAS.PROVINCIA
  AND TO_CHAR(ALBARANES.FECHA_ALBARAN, 'MM') IN ('04', '05', '06')
  AND LENGTH(PROVINCIAS.DESCRIPCION) > 6
GROUP BY LINEAS.ALBARAN, ALBARANES.CLIENTE, PROVINCIAS.DESCRIPCION, ALBARANES.FECHA_ENVIO
ORDER BY ALBARANES.FECHA_ENVIO, TOT_FRA
/
